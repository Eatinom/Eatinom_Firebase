...
class PopularChefs_Widget_State extends State<PopularChefs_Widget> {
  ...
  Widget build(BuildContext context) {
    ...
    switch (widget.mode) {
      case 'instant':
        return InstantAvailableChefs();
      case 'preorder':
        return PreAvailableChefs();
    }
  }
    ...
    try {
      return ValueListenableBuilder(
          ...
          builder: (context, value, child) {
            if (ChefData.topChefs_Instant != null &&
                ChefData.topChefs_Instant.length > 0) {
              List<Widget> lst = new List<Widget>();
              for (int i = 0; i < ChefData.topChefs_Instant.length; i++) {
                print('rating --- ' +
                    ChefData.topChefs_Instant[i]['Rating'].toString());
                if (i >= 1) {
                  break;
                } else {
  ...
  Widget PreAvailableChefs() {
    try {
      return ValueListenableBuilder(
          valueListenable: ChefData.notifier,
          builder: (context, value, child) {
            if (ChefData.topChefs_Pre != null &&
                ChefData.topChefs_Pre.isNotEmpty) {
              List<Widget> lst = new List<Widget>();
      ...
      return InkWell(
          onTap: () {
            Navigator.push(context,
                MaterialPageRoute(builder: (context) => Chef(chef: dt)));
          },
          child: Container(
              width: 190.0,
