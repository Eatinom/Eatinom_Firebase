...
class Login_State extends State<Login>{
  ...
  Widget bgCaurosel(){
    ...
    return CarouselSlider(
      ...
      item.contains('loginBG1') ?
      Container(padding: EdgeInsets.only(top: 100.0),
          child: Center(
          child: Image.asset(item, height: double.infinity)
          )
      ) : Container(child: Center(child: Image.asset(item, height: double.infinity))
      ),
      ).toList(),
    );
  }
      ...
      child: AnimatedContainer(
        ...
        child: Stack(children: [
            ...
            child: Container(
                ...
                child:RichText(textScaleFactor: 1.0,
                  text: TextSpan(
                    ...
                    style: TextStyle(fontSize: 19.0,height: 1.5,color: Colors.black),
                  /*  children: <TextSpan>[
          ...
          Align(
            ...
            child: Container(padding: EdgeInsets.only(left: 20.0,right: 20.0,top:0,bottom: 50),child: Row(
...
Align(
  ...
  child: Container(
   ...
   /*   Align(
      ...
      InkWell(
        child: Text('I accept terms of service and\nprivacy policy',textAlign: TextAlign.left,textScaleFactor: 1.1, style: TextStyle(fontSize: 18.0,height: 1.1, color: HexColor('#000000'), fontFamily: 'Product Sans', fontWeight: FontWeight.w500)),
        onTap: () {
          showDialog(
            context: context,
        ...
        /*  Align(
         ...
         //button
          Align(alignment: Alignment.bottomCenter,
            child: Container(padding: EdgeInsets.only(left: 10.0,right: 10.0,bottom: 10),child: sendOtpButton()),
          )
        ...
        child: AnimatedContainer(
          ...
          child: Stack(children: [
            ...
            Align(
              ...
              child: Container(padding: EdgeInsets.all(15.0),child: IconButton(
            ...
            Align(alignment: Alignment.topCenter,
              child: Container(
                ...
                child: phoneNumCtrl.text != null && phoneNumCtrl.text != ''? RichText(textScaleFactor: 1.0,
                    text: TextSpan(
                      text: 'OTP Verification\n',
                      style: TextStyle(fontSize: 19.0,height: 1.5,color: Colors.black),
                      children: <TextSpan>[
                        TextSpan(text: 'Please enter the OTP sent to '+phoneNumCtrl.text.substring(0,3)+'XXXXXX'+phoneNumCtrl.text.substring(phoneNumCtrl.text.length-2,phoneNumCtrl.text.length), style: TextStyle(fontSize: 11.0,color: Colors.grey)),
                      ],
                    )
  ...
  Widget phoneCode(){
    return Container(
      ...
      child: CupertinoTextField(textAlign: TextAlign.center,
...
*/
          ...
          RaisedButton(
            onPressed: () {
              ...
              if(phoneCodeCtrl.text.isEmpty){
                Flushbar(title:  "Error", message:  'Please enter correct Country Code', duration:  Duration(seconds: 3),)..show(context);
              }
              else if(phoneNumCtrl.text.isEmpty){
                Flushbar(title:  "Error", message:  'Please enter correct Mobile Number', duration:  Duration(seconds: 3),)..show(context);
              }
              else if(valuefirst == false){
                Flushbar(title:  "Error", message:  'Please accept our terms and conditions', duration:  Duration(seconds: 3),)..show(context);
              }
              else{
                // openLoadingBlock();
                var phnum = phoneCodeCtrl.text.toString() + phoneNumCtrl.text.toString();
                Flushbar(title:  "Info", flushbarPosition: FlushbarPosition.TOP,message:  'Sending OTP to given number - '+phnum, duration:  Duration(seconds: 3),)..show(context);
  ...
  Widget confirmOtpButton(){
    return Container(
      ...
      child: RaisedButton(
        onPressed: () async {
          FocusScope.of(context).unfocus();
          if(otpFieldCtrl1.text.isEmpty || otpFieldCtrl2.text.isEmpty || otpFieldCtrl3.text.isEmpty || otpFieldCtrl4.text.isEmpty || otpFieldCtrl5.text.isEmpty || otpFieldCtrl6.text.isEmpty){
            GlobalActions.showToast_Error("Error", 'Please enter correct OTP Code', context);
          }
          else{
            openLoadingBlock();
            print('verificationId -- '+verificationId);
            String smsCode = otpFieldCtrl1.text + otpFieldCtrl2.text +otpFieldCtrl3.text +otpFieldCtrl4.text +otpFieldCtrl5.text +otpFieldCtrl6.text ;
            ...
            if(authResult != null && authResult.user.uid != null){
              ...
              GlobalActions.showToast_Error("Error", 'Login failed, Please enter correct OTP.', context);
            }
          }
        },
        color: Colors.green,
  ...
  Widget otpNumberField(TextEditingController fCtrl){
    return Container(
      ...
      child: CupertinoTextField(
            ...
            cursorWidth: 5,cursorHeight: 1.0,
            maxLength: 1,
            onChanged: (value){
              if(value.isNotEmpty){
  ...
  void saveLoginAndDeviceInfo(String userID){
    try{
      ...
      AppConfig.storage.write(key: 'phoneID', value: phoneCodeCtrl.text+phoneNumCtrl.text);
      ...
      customer.doc(userID).set({
        ...
        'Referal_Code': userID.toUpperCase().substring(userID.length - 5 , userID.length),
      ...
      await FirebaseAuth.instance.verifyPhoneNumber(
        ...
        verificationFailed: (FirebaseAuthException e) {
          ...
          if (e.code == 'invalid-phone-number') {
            ...
            GlobalActions.showToast_Error('Error' , 'The provided phone number is not valid.' , context);
          }
        },
